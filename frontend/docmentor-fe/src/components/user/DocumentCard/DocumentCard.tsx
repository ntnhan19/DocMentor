...
export const DocumentCard: React.FC<DocumentCardProps> = ({
  ...
  if (view === "list") {
    return (
      // ✨ 3. Thêm onClick và className động vào div ngoài cùng
      <div
        onClick={() => onSelectionChange(id)}
        // ✨ Thêm `cursor-pointer` và hiệu ứng `ring` khi được chọn
      ...
      >
        ...
        {/* --- Toàn bộ JSX gốc của bạn cho List View được giữ nguyên --- */}   
           {" "}
        ...
        <div className="relative z-10 flex items-center gap-4">
          ...
          <div className="flex-grow min-w-0">
                       {" "}
            <h3 className="font-bold text-lg text-white mb-1 truncate group-hover:text-transparent group-hover:bg-gradient-to-r group-hover:from-primary group-hover:to-secondary group-hover:bg-clip-text transition-all duration-300">
              {title}
            </h3>
                       {" "}
  ...
  return (
    ...
    >
      {/* ✨ Thêm icon tick ở trên cùng (đặt ở góc trái để không đè lên nút xóa) */}
      {isSelected && (
      ...
      {/* --- Toàn bộ JSX gốc của bạn cho Grid View được giữ nguyên --- */}     {" "}
