...
import { Document } from "@/types/document.types"; // Import Document type

import Button from "@/components/common/Button";

interface DocumentUploadModalProps {
  isOpen: boolean;
  onClose: () => void; // FIX: Thay đổi từ () => void sang (doc: Document) => void
...
export const DocumentUploadModal: React.FC<DocumentUploadModalProps> = ({
  ...
  const handleUpload = async () => {
    ...
    setError(null);
    try {
      // Gọi API upload, hàm này trả về đối tượng Document đã được map
      const uploadedDoc = await documentService.uploadDocument(
        selectedFile,
        selectedFile.name
      ); // FIX: Truyền đối tượng tài liệu mới (uploadedDoc) vào callback
